...
Mesh::Mesh(const std::string &fname) : m_vertices(), m_faces() {
  ...
  while (ifs >> code) {
    ...
    boundsize = (double)glm::max(max_z - min_z, glm::max(max_x - min_x, max_y - min_y));
      std::cout<<"Center Bounding Box "<<glm::to_string(boundcenter)<<std::endl;
            std::cout<<"Size Bounding Box "<<boundsize<<std::endl;
    
    nonh_box = new NonhierBox(glm::vec3{boundcenter.x, boundcenter.y, boundcenter.z}, boundsize);
    ...
    for (auto triangle : m_faces) {
      ...
      if (t != EPSILON && t <= min_t && t <= hitInfo->t) {
        ...
        if (a >= 0 - EPSILON && a <= 1 + EPSILON && b >= 0 - EPSILON &&
          ...
          hitInfo->hitNormal = normalize(normal_vec);
...
Mesh::Mesh( std::vector<glm::vec3>& v, const std::vector<glm::vec3> &f, bool bv = true) :
	m_vertices(v) {
