...
template <typename T> void get_tuple(lua_State *L, int arg, T *data, int n) {
...
extern "C" int gr_bump_cmd(lua_State *L) {
  gr_node_ud *selfdata = (gr_node_ud *)luaL_checkudata(L, 1, "gr.node");
  luaL_argcheck(L, selfdata != 0, 1, "Node expected");
...
extern "C" int gr_mirror_cmd(lua_State *L) {
  gr_node_ud *selfdata = (gr_node_ud *)luaL_checkudata(L, 1, "gr.node");
  luaL_argcheck(L, selfdata != 0, 1, "Node expected");
...
static const luaL_Reg grlib_node_methods[] = {
    ...
    {"render", gr_render_cmd},
    {"set_bump", gr_bump_cmd},
    {"set_mirror", gr_mirror_cmd},
    {0, 0}};
