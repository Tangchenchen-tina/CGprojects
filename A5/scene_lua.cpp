...
extern "C" int gr_node_set_material_cmd(lua_State *L) {
  ...
  gr_material_ud *matdata =
      (gr_material_ud *)luaL_checkudata(L, 2, "gr.material");
...
extern "C" int gr_lnode_cmd(lua_State *L) {
  gr_node_ud *selfdata = (gr_node_ud *)luaL_checkudata(L, 1, "gr.node");
  luaL_argcheck(L, selfdata != 0, 1, "Node expected");

  int layers = luaL_checknumber(L, 2);
  int base_num = luaL_checknumber(L, 3);
  int horizon = luaL_checknumber(L, 4);
  int angle = luaL_checknumber(L, 5);
  float parent_scale = luaL_checknumber(L, 6);

  SceneNode *self = (selfdata->node);
  self->set_lnode(layers, base_num, horizon, angle, parent_scale);
  return 0;
}
...
static const luaL_Reg grlib_functions[] = {{"node", gr_node_cmd},
                                           {"joint", gr_joint_cmd},
                                           {"mesh", gr_mesh_cmd},
...
SceneNode *import_lua(const std::string &filename) {
  ...
  if (luaL_loadfile(L, filename.c_str()) || lua_pcall(L, 0, 1, 0)) {
    std::cerr << "Error loading " << filename << ": " << lua_tostring(L, -1)
              << std::endl;
    return 0;
  }
